...
for (const { out, src, minify } of BUNDLES) {
  ...
  // A stamp over the sources, so "is my browser running the latest build?" has
  // an answer you can read instead of infer. It cost a round trip not having
  // one. Hashed before substitution, or it would be hashing itself.
  const build = createHash("sha1").update(joined).digest("hex").slice(0, 7);
  joined = joined.replaceAll("__PIXL_VERSION__", VERSION)
                 .replaceAll("__PIXL_BUILD__", build);
  ...
  console.log(`${out.padEnd(20)} ${src.length} files → ${kb(result.code.length)}  build ${build}`);
}
